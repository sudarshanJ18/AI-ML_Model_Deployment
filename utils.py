...
# Initialize FaceNet and MTCNN once (they're expensive to load)
facenet = FaceNet()
detector = MTCNN()
...
def get_embedding(face_img):
    ...
    # Get embedding (FaceNet returns a list of embeddings, we take the first one)
    embedding = facenet.embeddings(np.expand_dims(face_img, axis=0))[0]
    return embedding
